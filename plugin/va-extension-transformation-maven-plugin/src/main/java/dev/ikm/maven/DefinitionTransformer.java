        ...
        try (Stream<String> lines = Files.lines(definitionFile.toPath())) {
            lines.skip(1) //skip first line, i.e. header line
                    ...
                    .forEach(data -> {
                        ...
                        long epochTime = VaExtensionUtility.timestampToEpochSeconds(data[EFFECTIVE_TIME]);
                        EntityProxy.Concept moduleConcept = EntityProxy.Concept.make(PublicIds.of(VaExtensionUtility.generateUUID(namespace, data[MODULE_ID])));
                        ...
                        if (!data[ID].equals(previousRowId)) {
                            ...
                            previousReferencedConcept = EntityProxy.Concept.make(PublicIds.of(VaExtensionUtility.generateUUID(namespace, data[CONCEPT_ID])));
                            previousDefinitionSemantic = EntityProxy.Semantic.make(PublicIds.of(VaExtensionUtility.generateUUID(namespace, data[ID])));
                        }

                        EntityProxy.Concept languageConcept = TransformationHelper.getLanguageConcept(data[LANGUAGE_CODE]);
                        EntityProxy.Concept caseSignificanceConcept = EntityProxy.Concept.make(PublicIds.of(VaExtensionUtility.generateUUID(namespace, data[CASE_SIGNIFICANCE_ID])));
                        EntityProxy.Concept descriptionTypeConcept = EntityProxy.Concept.make(PublicIds.of(VaExtensionUtility.generateUUID(namespace, data[TYPE_ID])));
