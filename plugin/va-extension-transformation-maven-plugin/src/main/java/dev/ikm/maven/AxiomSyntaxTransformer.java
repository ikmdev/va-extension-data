...
public class AxiomSyntaxTransformer extends AbstractTransformer {
        ...
        try (Stream<String> lines = Files.lines(axiomFile.toPath())) {
            lines.skip(1)
                    ...
                    .forEach(data -> {
                        ...
                        EntityProxy.Concept module = EntityProxy.Concept.make(PublicIds.of(VaExtensionUtility.generateUUID(namespace, data[MODULE_ID])));
    ...
    private void configureSemanticsForConcept(Session session, String[] columns) {
        String owlExpressionWithPublicIds = VaExtensionUtility.owlAxiomIdsToPublicIds(namespace, columns[OWL_EXPRESSION]);

        EntityProxy.Concept concept = EntityProxy.Concept.make(PublicIds.of(VaExtensionUtility.generateUUID(namespace, columns[REFERENCED_COMPONENT_ID])));
        EntityProxy.Semantic axiomSemantic = EntityProxy.Semantic.make(PublicIds.of(VaExtensionUtility.generateUUID(namespace, columns[ID])));
