        ...
        try (Stream<String> lines = Files.lines(descriptionFile.toPath())) {
            lines.skip(1) //skip first line, i.e. header line
                    ...
                    .forEach(data -> {
                        ...
                        EntityProxy.Concept moduleId = EntityProxy.Concept.make(PublicIds.of(VaExtensionUtility.generateUUID(namespace, data[MODULE_ID])));
                        ...
                        EntityProxy.Concept caseSensitivityConcept = EntityProxy.Concept.make(PublicIds.of(VaExtensionUtility.generateUUID(namespace, data[CASE_SIGNIFICANCE])));

                        EntityProxy.Concept concept = EntityProxy.Concept.make(PublicIds.of(VaExtensionUtility.generateUUID(namespace, data[CONCEPT_ID])));
                        EntityProxy.Semantic definitionSemantic = EntityProxy.Semantic.make(PublicIds.of(VaExtensionUtility.generateUUID(namespace, data[ID])));
